...
class App {
    ...
    private initializeRoutes() {
        ...
        // Conversation membership verification for the GenAI/RAG backend
        this.app.use(
    ...
    private async connectRedis() {
        ...
        try {
            ...
            console.log(
                `Redis memory usage: ${usedMemory} bytes`
            );
        } catch (err) {
    ...
    private initializeSockets() {
        this.chatNamespace.on("connect", async (socket: CustomSocket) => {
            console.log(
                `User ${socket?.user?.name} with UUID ${socket?.user?.socket_id} connected`
            );
        ...
        this.callsNamespace.on("connect", async (socket: CustomSocket) => {
            console.log(
                `Calls namespace connected with id: ${socket.id}`
            );
    ...
    public listen() {
        this.server.listen(this.port, () => {
            console.log(
                `Server is running on port number ${this.port}`
            );
        });
